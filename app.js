const APP_VERSION='family-cloud-v20.1';
...
const KEY='tokyoFreeChoice';
const SCHEMA_VERSION=207;
const VERSION_KEY='tokyoFreeChoiceSchemaVersion';
...
function makeBackup(label='auto'){
  try{
    ...
    localStorage.setItem(`${KEY}_backup_${label}_${stamp}`,raw);
  }catch(err){
    console.warn('建立備份失敗',err);
  }
}
...
function normalizeLegacyState(input){
  ...
  s.spots=(Array.isArray(s.spots)?s.spots:[]).map(x=>({
    ...x,
  ...
  if(!Object.keys(s.wishlists).length && s.selected.length){
    s.wishlists['destination:東京']=[...s.selected];
  }
  s.trips=(Array.isArray(s.trips)?s.trips:[]).filter(Boolean).map(t=>({
    ...t,
    destination:t.destination||'東京',
    days:t.days&&typeof t.days==='object'&&!Array.isArray(t.days)?t.days:{},
    itemNotes:t.itemNotes&&typeof t.itemNotes==='object'&&!Array.isArray(t.itemNotes)?t.itemNotes:{},
    flights:t.flights&&typeof t.flights==='object'&&!Array.isArray(t.flights)?t.flights:{
...
function mergeStates(localState,remoteState){
  const local=normalizeLegacyState(localState);
  const remote=normalizeLegacyState(remoteState);
  const merged={...local,...remote};

  const spotMap=new Map();
  for(const s of [...local.spots,...remote.spots]){
    const key=s.mapUrl||`${String(s.name||'').trim().toLowerCase()}|${s.id}`;
    if(!spotMap.has(key))spotMap.set(key,s);
    else spotMap.set(key,{...spotMap.get(key),...s});
  }
  merged.spots=[...spotMap.values()];

  merged.selected=[...new Set([...(local.selected||[]),...(remote.selected||[])])];

  const tripMap=new Map();
  for(const t of [...local.trips,...remote.trips]){
    if(!t||!t.id)continue;
    ...
    const existingTime=existing.updatedAt||existing.created||existing.createdAt||'';
    const newTime=t.updatedAt||t.created||t.createdAt||'';
    tripMap.set(t.id,newTime>=existingTime?{...existing,...t}:{...t,...existing});
  }
  merged.trips=[...tripMap.values()];

  if(!merged.currentTripId && local.currentTripId)merged.currentTripId=local.currentTripId;
  return merged;
}


const REGION_SEED_VERSION=1;
const REGION_SEED_KEY='tokyoFreeChoiceRegionSeedVersion';

const REGION_KEYWORDS={
  '淺草／押上':['淺草','浅草','雷門','仲見世','晴空塔','スカイツリー','押上','墨田','隅田川','合羽橋'],
  '上野／谷中':['上野','御徒町','阿美橫','アメ横','谷中','日暮里','鶯谷'],
  '東京站／日本橋':['東京車站','東京站','東京駅','丸之內','丸の内','日本橋','京橋','八重洲','KITTE'],
  '銀座／築地':['銀座','築地','東銀座','有樂町','有楽町','日比谷'],
  '新宿':['新宿','歌舞伎町','東京都廳','都庁','思出橫丁','思い出横丁','大久保'],
  '澀谷／原宿／表參道':['澀谷','渋谷','原宿','表參道','表参道','明治神宮','青山','代官山','惠比壽','恵比寿'],
  '池袋':['池袋','太陽城','サンシャイン'],
  '秋葉原／神田':['秋葉原','神田','御茶之水','御茶ノ水'],
  '六本木／麻布':['六本木','麻布','麻布台','赤坂','虎之門','虎ノ門'],
  '東京鐵塔／芝公園':['東京鐵塔','東京タワー','芝公園','增上寺','増上寺','濱松町','浜松町'],
  '台場／豐洲':['台場','お台場','豐洲','豊洲','teamLab Planets','DiverCity','有明'],
  '吉祥寺／三鷹':['吉祥寺','三鷹','井之頭','井の頭','吉卜力'],
  '世田谷':['世田谷','豪德寺','豪徳寺','下北澤','下北沢'],
  '舞濱／迪士尼':['舞濱','舞浜','迪士尼','Disney','ディズニー'],
  '中目黑／自由之丘':['中目黑','中目黒','自由之丘','自由が丘','目黑川','目黒川'],
  '品川／羽田':['品川','羽田','蒲田','天王洲'],
  '大阪－難波／心齋橋':['難波','なんば','心齋橋','心斎橋','道頓堀','道頓崛','黑門市場','黒門市場'],
  '大阪－梅田':['梅田','大阪站','大阪駅','北新地'],
  '大阪－天王寺／新世界':['天王寺','新世界','通天閣','阿倍野'],
  '大阪－大阪城':['大阪城','森之宮'],
  '京都－京都站':['京都站','京都駅'],
  '京都－祇園／清水寺':['祇園','清水寺','八坂神社','花見小路'],
  '京都－嵐山':['嵐山','渡月橋','竹林'],
  '京都－伏見':['伏見','稻荷','稲荷']
};

function inferInitialRegion(spot){
  const current=String(spot.area||'').trim();
  const genericAreas=['','未標記','待確認','自訂','Google Maps 東京清單','Google Maps 日本東京清單'];
  if(current && !genericAreas.includes(current)) return current;

  const haystack=[
    spot.name, spot.note, spot.cat, spot.mapUrl, spot.destination
...
function seedInitialRegions(targetState){
  if(!targetState||!Array.isArray(targetState.spots))return targetState;
  targetState.regionsByDestination=
    targetState.regionsByDestination &&
    typeof targetState.regionsByDestination==='object' &&
    !Array.isArray(targetState.regionsByDestination)
      ? targetState.regionsByDestination
      : {};

  for(const s of targetState.spots){
    if(!s)continue;
    const inferred=inferInitialRegion(s);
    s.area=inferred;
    const destination=s.destination||'東京';
    if(inferred!=='待確認'&&inferred!=='未標記'){
      const list=Array.isArray(targetState.regionsByDestination[destination])
        ? targetState.regionsByDestination[destination]
        : [];
      if(!list.includes(inferred))list.push(inferred);
      targetState.regionsByDestination[destination]=list;
    }
  }
  return targetState;
}

function runInitialRegionSeed(){
  try{
    const seededVersion=Number(localStorage.getItem(REGION_SEED_KEY)||0);
    if(seededVersion>=REGION_SEED_VERSION)return;

    const raw=localStorage.getItem(KEY);
    if(raw){
      makeBackup('before-region-seed');
      const existing=JSON.parse(raw);
      localStorage.setItem(KEY,JSON.stringify(seedInitialRegions(existing)));
    }
    localStorage.setItem(REGION_SEED_KEY,String(REGION_SEED_VERSION));
  }catch(err){
    console.error('初步區域預選失敗',err);
  }
}

runInitialRegionSeed();

(function migrateSafely(){
  try{
    const oldVersion=Number(localStorage.getItem(VERSION_KEY)||0);
    if(oldVersion<SCHEMA_VERSION){
      makeBackup(`before-v${SCHEMA_VERSION}`);
      const existing=JSON.parse(localStorage.getItem(KEY)||'null');
      if(existing){
        localStorage.setItem(KEY,JSON.stringify(normalizeLegacyState(existing)));
      }
      localStorage.setItem(VERSION_KEY,String(SCHEMA_VERSION));
    }
  }catch(err){
    console.error('資料移轉失敗',err);
  }
})();
let state=normalizeLegacyState(seedInitialRegions(JSON.parse(localStorage.getItem(KEY)||'null')||{
  page:'spots',spots:defaultSpots,selected:[],trips:[],currentTripId:null,query:''
}));
state.spots=mergeStates(
  {spots:defaultSpots,selected:[],trips:[]},
  {spots:state.spots,selected:state.selected,trips:state.trips}
).spots;
...
const fmt=d=>{
  ...
  return `${x.getMonth()+1}/${x.getDate()}（${'日一二三四五六'[x.getDay()]}）`;
};
const dates=(a,b)=>{
  const result=[];
...
const DESTINATIONS=['東京','大阪','京都','沖繩','北海道','福岡','名古屋','其他'];

function wishlistKeyForDestination(destination){
  return `destination:${destination||'東京'}`;
}

function wishlistKeyForTrip(tripId){
  return `trip:${tripId}`;
}

function currentWishlistKey(){
  return state.activeWishlistKey || wishlistKeyForDestination(state.activeDestination);
}

function wishlistIds(key=currentWishlistKey()){
  const ids=state.wishlists?.[key];
  return Array.isArray(ids)?ids:[];
}

function setWishlistIds(ids,key=currentWishlistKey()){
  state.wishlists=state.wishlists||{};
  state.wishlists[key]=[...new Set(ids)];
  // 保留舊欄位相容性，避免舊版或雲端資料讀取失敗。
  state.selected=[...new Set(Object.values(state.wishlists).flat().filter(Number.isFinite))];
}

function wishlistLabel(key=currentWishlistKey()){
  if(key.startsWith('trip:')){
    const t=state.trips.find(x=>x.id===key.slice(5));
    return t?`${t.name}－想去`:'旅程想去';
  }
  return `${key.replace('destination:','')}想去`;
}

function setDestination(destination){
  state.activeDestination=destination;
  state.activeWishlistKey=wishlistKeyForDestination(destination);
  save();
  render();
}

function setWishlistKey(key){
  state.activeWishlistKey=key;
  if(key.startsWith('destination:'))state.activeDestination=key.replace('destination:','');
  save();
  render();
}
...
function regionModalHtml({mode='manage',spotId=null}={}){
  const s=spotId!==null?spot(spotId):null;
  const destination=s?.destination||state.activeDestination||'東京';
  const regions=availableRegions(destination);
  const selected=s?.area&&s.area!=='未標記'?s.area:'';
    ...
    <div class="modal region-modal">
      <div class="modal-head">
        <div>
          ...
          <h2>${mode==='select'?'選擇景點區域':'管理區域'}</h2>
        </div>
        <button class="modal-close" onclick="$('modal').innerHTML=''">✕</button>
      </div>

      ${mode==='select'?`
        <p class="sub">請選擇「${esc(s?.name||'此景點')}」所在區域</p>
      `:`
        <p class="sub">新增、重新命名或刪除區域，所有家庭成員都會同步看到。</p>
      `}

      <div class="region-chip-list">
        ${regions.length?regions.map(r=>`
          <div class="region-chip-row ${selected===r?'selected':''}">
            <button class="region-chip-main" onclick="${mode==='select'
              ? `selectSpotRegion(${spotId},'${String(r).replace(/'/g,"\'")}')`
              : `filterByRegion('${String(r).replace(/'/g,"\'")}')`}">
              ${esc(r)}
            </button>
            ${mode==='manage'?`
              <button class="region-chip-edit" onclick="renameRegion('${String(r).replace(/'/g,"\'")}')">✎</button>
              <button class="region-chip-delete" onclick="deleteRegion('${String(r).replace(/'/g,"\'")}')">🗑</button>
            `:''}
          </div>
        `).join(''):`<div class="empty">尚未建立任何區域</div>`}
      </div>

      ${mode==='select'&&selected?`
        <button class="big" onclick="selectSpotRegion(${spotId},'未標記')">清除區域標記</button>
      `:''}

      <div class="region-add-box">
        <label>新增區域</label>
        <div class="region-add-row">
          <input id="newRegionName" placeholder="例如：淺草、難波、梅田">
          <button class="primary" onclick="addRegionFromModal('${destination.replace(/'/g,"\'")}',${spotId===null?'null':spotId},'${mode}')">新增</button>
        </div>
      </div>
    </div>
  </div>`;
...
function reviewUnconfirmedRegions(){
  const destination=state.activeDestination||'東京';
  const pending=state.spots.filter(s=>
    (s.destination||'東京')===destination &&
    ['待確認','未標記',''].includes(String(s.area||'').trim())
  );
  if(!pending.length)return toast('這個目的地目前沒有待確認的景點');
  state.filterRegion='待確認';
  save();
  render();
  toast(`共有 ${pending.length} 個景點待確認`);
}
...
function addRegionFromModal(destination,spotId,mode){
  const input=$('newRegionName');
  const name=(input?.value||'').trim();
  if(!name)return toast('請輸入區域名稱');

  const regions=availableRegions(destination);
  if(!regions.includes(name)){
    saveRegionList(destination,[...regions,name]);
  }
...
function selectSpotRegion(id,region){
  const s=spot(id);
  ...
  if(window.familyCloud?.updateSpotRegion){
    window.familyCloud.updateSpotRegion(id,region);
  }else{
    save();
  }
  toast(region==='未標記'?'已清除區域':'已標記為「'+region+'」');
}
...
function deleteRegion(name){
  const destination=state.activeDestination||'東京';
  if(!confirm(`確定刪除「${name}」？使用此區域的景點會改為「未標記」。`))return;

  state.spots.forEach(s=>{
    if((s.destination||'東京')===destination&&s.area===name)s.area='未標記';
  });
...
function spotsPage(){
  ...
  <div class="region-filter">
    ...
    <button onclick="reviewUnconfirmedRegions()">待確認</button>
  </div>
...
function wantedPage(){
  const key=currentWishlistKey();
  const ids=wishlistIds(key);
  const list=ids.map(spot).filter(Boolean);
  const destinationOptions=DESTINATIONS.map(d=>
    `<option value="${wishlistKeyForDestination(d)}" ${key===wishlistKeyForDestination(d)?'selected':''}>${d}想去</option>`
  ).join('');
  const tripOptions=state.trips.map(t=>
    `<option value="${wishlistKeyForTrip(t.id)}" ${key===wishlistKeyForTrip(t.id)?'selected':''}>${esc(t.name)}（${esc(t.destination||'東京')}）</option>`
  ).join('');

  return `<div class="titleRow">
    <div><h1>想去清單</h1><p class="sub">可依目的地或旅程分開收藏。</p></div>
  </div>
  <label>目前查看分類</label>
  <select class="wishlist-select" onchange="setWishlistKey(this.value)">
    <optgroup label="依目的地">${destinationOptions}</optgroup>
    ${tripOptions?`<optgroup label="依旅程">${tripOptions}</optgroup>`:''}
  </select>
  <p class="sub">共 ${list.length} 個景點</p>
  ${list.length?
    `<div class="grid">${spotCards(list)}</div>
     <button class="big primary" onclick="state.trips.length?go('trips'):go('newTrip')">安排到旅程</button>`:
    `<div class="empty">這個分類尚未加入景點<br><button onclick="go('spots')">開始找景點</button></div>`
  }`;
}
function newTripPage(){return `<h1>建立旅程</h1><p class="sub">設定名稱與日期，系統會自動建立每天行程。</p><form class="form" onsubmit="createTrip(event)">
<label>旅程名稱</label><input id="tn" required placeholder="例如：爸媽大阪自由行">
<label>目的地</label>
<select id="td">${DESTINATIONS.map(d=>`<option ${d===state.activeDestination?'selected':''}>${d}</option>`).join('')}</select>
<div class="cols"><div><label>開始日期</label><input id="ts" type="date" required></div><div><label>結束日期</label><input id="te" type="date" required></div></div><label>備註</label><textarea id="tt" placeholder="例如：步調輕鬆、住宿在淺草"></textarea><button class="big primary">建立旅程</button></form>`}
function createTrip(e){e.preventDefault();let a=$('ts').value,b=$('te').value;if(b<a)return toast('結束日期不能早於開始日期');let days={};dates(a,b).forEach(d=>days[d]=[]);let t={
  ...
  state.activeWishlistKey=wishlistKeyForTrip(t.id);
  setWishlistIds([...wishlistIds(wishlistKeyForDestination(t.destination))],wishlistKeyForTrip(t.id));
  save();
  go('planner')
}
function tripsPage(){let now=localToday(),active=state.trips.filter(t=>!t.archived&&t.end>=now),history=state.trips.filter(t=>t.archived||t.end<now);return `<div class="titleRow"><div><h1>我的旅程</h1><p class="sub">旅程名稱、日期與每天安排都會保留。</p></div><button class="primary" onclick="go('newTrip')">＋新增</button></div><h2>目前與未來</h2>${tripList(active)}<h2>歷史旅程</h2>${tripList(history,true)}${backupPanel()}`}
function tripList(list,hist=false){return list.length?`<div class="tripList">${list.map(t=>{let n=Object.values((t&&t.days)||{}).flat().length;return `<article class="tripCard"><small>${hist?'歷史旅程':'目前旅程'}</small><h3>${esc(t.name)}</h3><p>📍 ${esc(t.destination||'東京')}　📅 ${fmt(t.start)}－${fmt(t.end)}</p><p>已安排 ${n} 個景點</p><div class="actions"><button onclick="openTrip('${t.id}')">查看／編輯</button><button onclick="archiveTrip('${t.id}')">${t.archived?'移回目前':'封存'}</button><button class="danger" onclick="deleteTrip('${t.id}')">刪除</button></div></article>`}).join('')}</div>`:`<div class="empty">尚無資料</div>`}
function openTrip(id){
  const t=state.trips.find(x=>x&&x.id===id);
  if(!t)return toast('找不到這筆旅程');
...
function archiveTrip(id){
  ...
  save();
  render();
  toast(t.archived?'已封存':'已移回目前旅程');
}
function deleteTrip(id){
  const t=state.trips.find(x=>x&&x.id===id);
  if(!t)return toast('找不到這筆旅程');
  if(confirm(`刪除「${t.name}」？`)){
    state.trips=state.trips.filter(x=>x&&x.id!==id);
...
function plannerPage(){let t=trip();if(!t)return `<div class="empty">請先選擇旅程<br><button onclick="go('trips')">回旅程列表</button></div>`;let ds=dates(t.start,t.end);return `<button class="back" onclick="go('trips')">← 回我的旅程</button><div class="titleRow"><div><small>正在安排</small><h1>${esc(t.name)}</h1><p class="sub">${fmt(t.start)}－${fmt(t.end)}</p></div><button onclick="editTrip()">編輯資料</button></div>${t.note?`<div class="note">📝 ${esc(t.note)}</div>`:''}<div class="dayTabs">${ds.map((d,i)=>`<button class="${i===0?'on':''}" onclick="showDay('${d}',this)">第${i+1}天<br><b>${fmt(d)}</b><br><small>${t.days[d].length}個</small></button>`).join('')}</div><div id="day">${dayPanel(ds[0])}</div><div id="modal"></div>`}
...
function addPage(){
  ...
  return `<section>
    <h1>手動新增景點</h1>
    <p class="sub">可自行新增餐廳、景點、住宿或朋友推薦地點。</p>

    <div class="tip">
      <b>建議操作方式</b>
      <p>先到 Google Maps 找到地點，按「分享」複製連結，再貼到下方。</p>
    </div>

    <form class="form" onsubmit="addCustom(event)">
      <label>目的地</label>
<select id="ad">${DESTINATIONS.map(d=>`<option ${d===state.activeDestination?'selected':''}>${d}</option>`).join('')}</select>
<label>景點名稱</label>
      <input id="an" required placeholder="例如：六歌仙燒肉">

      <label>Google Maps 分享連結</label>
      <input id="au" inputmode="url" placeholder="https://maps.app.goo.gl/...">

      <div class="cols">
        <div>
          <label>所在區域</label>
          <input id="aa" placeholder="例如：新宿">
        </div>
        <div>
          <label>類型</label>
          <select id="ac">
      ...
      <div class="cols">
        <div>
          ...
          <select id="am">
            ...
            <option value="90" selected>1.5 小時</option>
        ...
        <div>
          ...
          <select id="aw">
            ...
            <option>普通</option>
            <option>較多步行</option>
            <option selected>待確認</option>
          </select>
        </div>
      </div>

      <label>景點備註</label>
      <textarea id="at" placeholder="例如：需要預約、朋友推薦、晚餐候選"></textarea>

      <button class="big primary">儲存並加入想去清單</button>
    </form>

    <h2>我自行新增的景點</h2>
    ${manual.length ? `<div class="manualList">${manual.map(s=>`
      <article class="manualCard">
        <div>
          <h3>${esc(s.name)}</h3>
          <p>${esc(s.area||'自訂')}・${esc(s.cat||'其他')}・${s.minutes||90} 分鐘</p>
          ${s.note?`<small>${esc(s.note)}</small>`:''}
        </div>
        <div class="manualBtns">
          <button onclick="mapOpen(${s.id})">地圖</button>
          <button class="danger" onclick="deleteManual(${s.id})">刪除</button>
        </div>
      </article>`).join('')}</div>` : `<div class="empty">尚未自行新增景點</div>`}
  </section>`;
}
...
function addCustom(e){
  ...
  const url=$('au').value.trim();
  if(!name)return toast('請輸入景點名稱');
  if(url && !/^https?:\/\//i.test(url))return toast('Google Maps 連結格式不正確');

  const s={
    id:Date.now(),
    name,
    destination:$('ad').value,
    area:$('aa').value.trim()||'自訂',
    cat:$('ac').value,
    icon:'📍',
    minutes:Number($('am').value),
    walk:$('aw').value,
    indoor:false,
    note:$('at').value.trim()||'自行新增的景點',
    mapUrl:url,
    source:'manual'
  };

  state.spots.unshift(s);
  state.activeDestination=s.destination;
  state.activeWishlistKey=wishlistKeyForDestination(s.destination);
  const ids=wishlistIds();
  if(!ids.includes(s.id))ids.push(s.id);
  setWishlistIds(ids);
  save();
  toast('已新增並加入想去清單');
  setTimeout(()=>go('wanted'),400);
}

function deleteManual(id){
  const s=spot(id);
  if(!s || s.source!=='manual')return;
  if(!confirm(`確定刪除「${s.name}」嗎？`))return;

  state.spots=state.spots.filter(x=>x.id!==id);
  state.selected=state.selected.filter(x=>x!==id);
  Object.keys(state.wishlists||{}).forEach(k=>{
    state.wishlists[k]=(state.wishlists[k]||[]).filter(x=>x!==id);
  });
  ...
  state.trips.forEach(t=>{
    ...
    if(t.itemNotes){
      Object.keys(t.itemNotes).forEach(k=>{
        if(k.endsWith(':'+id))delete t.itemNotes[k];
      });
    }
  });
...
function exportData(){
  try{
    ...
    a.href=url;
    a.download=`東京旅程備份_${new Date().toISOString().slice(0,10)}.json`;
    a.click();
...
function importData(){
  ...
  input.onchange=async()=>{
    ...
    try{
      ...
      const incoming=payload.appState||payload;
