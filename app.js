const APP_VERSION='family-cloud-v11';
...
let state=normalizeLegacyState(JSON.parse(localStorage.getItem(KEY)||'null')||{
...
} ${s.note||''} ${s.cat||''}`.toLowerCase().includes(q));$('count').textContent=list.length+' 個地點';$('cards').innerHTML=spotCards(list)}
function wantedPage(){
